...
@Component({
  selector: 'app-create-latest-updates',
  templateUrl: './create-latest-updates.component.html',
  styleUrls: ['./create-latest-updates.component.scss']
})
export class CreateLatestUpdatesComponent implements OnInit {
  name: any;
  image: any;
  file: any;
  description: any;
  ...
  submit(frm: NgForm) {
    ...
    if (f.invalid) {
      ...
      this.api.post("latestUpdate", formData).subscribe((resp: any) => {
        if (resp) {
          console.log(resp);
          alert("update created successfully");
          this.router.navigate(["/content/latest-update-list"]);
        }
      });
    }
  }
