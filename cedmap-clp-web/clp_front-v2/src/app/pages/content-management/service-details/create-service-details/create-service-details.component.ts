...
@Component({
  selector: 'app-create-service-details',
  templateUrl: './create-service-details.component.html',
  styleUrls: ['./create-service-details.component.scss']
})
export class CreateServiceDetailsComponent implements OnInit {
  name: any;
  serviceName:any;
  image1: any;
  image2: any;
  ...
  submit(frm: NgForm) {
    ...
    if (f.invalid) {
      ...
      if (this.img_link4) {
        formData.append('image4', this.img_link4);
      }
      ...
      this.api.post("servicedetail", formData).subscribe((resp: any) => {
        if (resp) {
          console.log(resp);
...
//   const file = event.target.files[0];
//   if (file) {
//     this[`selectedImage${imageIndex}`] = file;
//     this.uploadImage(imageIndex);
//   }
// }

// uploadImage(imageIndex: number) {
//   const selectedImage = this[`selectedImage${imageIndex}`];
//   if (selectedImage) {
//     this.fs.uploadFile(selectedImage).subscribe((res: any) => {
//       if (res.type == HttpEventType.Response) {
//         const body: any = res.body;
//         this[`img_link${imageIndex}`] = body.file.path;
//       }
//     });
//   } else {
   ...
   pdf() {
      ...
      getServiceList(): void {
        this.api.get("service", {}).subscribe((resp: any) => {
          if (resp) {
            this.serviceList = resp.data; // Assuming the service list is in `resp.data`
          }
        });
      }
}
