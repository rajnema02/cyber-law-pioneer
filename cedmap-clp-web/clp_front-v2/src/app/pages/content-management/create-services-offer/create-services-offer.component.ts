...
@Component({
  selector: 'app-create-services-offer',
  templateUrl: './create-services-offer.component.html',
  styleUrls: ['./create-services-offer.component.scss']
})
export class CreateServicesOfferComponent implements OnInit {
  name: any;
  image: any;
  image1: any;
  image2: any;
  ...
  selectedPdf: File | null = null; // To store the selected PDF file
  ...
  submit(frm: NgForm) {
    ...
    if (f.invalid) {
      ...
      this.api.post("servicesOffers", formData).subscribe((resp: any) => {
        if (resp) {
          console.log(resp);
          alert("Service Offer created successfully");
          this.router.navigate(["/content/service-offers"]);
        }
      });
    }
  }
