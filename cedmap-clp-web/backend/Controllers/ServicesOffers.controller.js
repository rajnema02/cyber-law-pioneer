const Model = require('../Models/ServicesOffers.model');
...
module.exports = {
    ...
    update: async (req, res, next) => {
        try {
            ...
            if (!existingService) {
                throw createError.NotFound('Service or offer not found');
            }
            ...
            if (req.files) {
                if (req.files.image) {
                    updateData.image_url = req.files.image[0].path;
                }
                if (req.files.file) {
                    updateData.file_url = req.files.file[0].path;
                }
            }
    ...
    getById: async (req, res, next) => {
        try {
            ...
            if (!mongoose.Types.ObjectId.isValid(id)) {
                return res.status(400).json({ error: 'Invalid service or offer ID format' });
            }

            const service = await Model.findById(id);
            if (!service || service.is_inactive) {
                return res.status(404).json({ error: 'Service or offer not found or inactive' });
            }
