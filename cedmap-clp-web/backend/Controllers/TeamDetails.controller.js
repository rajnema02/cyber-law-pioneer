const Model = require('../Models/TeamDetails.model')
   ...
   delete: async (req, res, next) => {
    ...
    update: async (req, res, next) => {
        try {
            ...
            const existingTeamMember = await Model.findById(id);
            ...
            const updatedTeamMember = await Model.findByIdAndUpdate(id, updateData, { new: true });
    ...
    getById: async (req, res, next) => {
        try {
            ...
            const teamMember = await Model.findById(id);
            if (!teamMember || teamMember.is_inactive) {
                return res.status(404).json({ error: 'Team member not found or inactive' });
            }

            console.log('Requested team member ID:', id);
